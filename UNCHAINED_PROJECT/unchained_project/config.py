...
def _apply_toml(cfg: AppConfig, data: dict):
    ...
    for key in ("step_grade", "smoothing", "min_difficulty_scale", "downhill_scale", "startup_resistance_ramp_s"):
        if key in gear:
...
def _apply_env(cfg: AppConfig):
    ...
    if v := os.getenv("UNCHAINED_PROJECT_PORT"):
        cfg.port = int(v)
    if v := os.getenv("UNCHAINED_PROJECT_HOST"):
        cfg.host = v
    if v := os.getenv("UNCHAINED_PROJECT_DEBUG"):
        cfg.debug = v.lower() in ("1", "true", "yes")
    if v := os.getenv("UNCHAINED_PROJECT_DATA_DIR"):
        cfg.data_dir = Path(v)
