...
from fuckzwift.state import state, ride_data, new_ride_data, route_points, finalize_lock
import fuckzwift.state as app_state
...
def finalize_ride(strava_service, profile, config, upload_to_strava=True):
    ...
    from fuckzwift.storage.activities import save_activity
    from fuckzwift.storage.export import export_ride_to_gpx
    ...
    if upload_to_strava and gpx_path and strava_service.status().get("connected"):
        try:
            strava_result = strava_service.upload_activity(
                gpx_path,
                payload["route_name"],
                description=f"Uploaded from FUCK ZWIFT on {datetime.now().strftime('%Y-%m-%d')}",
...
async def do_start_ride(gear_system, physics_engine, profile, strava_service, config):
    ...
    from fuckzwift.routes.gpx import load_gpx, compute_slopes, get_total_distance, get_slope_at_distance
    from fuckzwift.ride.simulation import get_elevation_at_distance
    from fuckzwift.ble.ftms import set_simulation_params
    from fuckzwift.ble.manager import get_trainer_client
