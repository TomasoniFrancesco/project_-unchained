...
from fuckzwift.state import state, route_points
from fuckzwift.ble.manager import ble_submit, do_scan, do_connect_trainer, do_connect_controller
from fuckzwift.ride.engine import (
    do_start_ride, do_stop_ride, toggle_pause_ride, finalize_ride,
)
from fuckzwift.routes.registry import discover_routes
from fuckzwift.routes.gpx import load_gpx, get_total_distance
...
def api_strava_status():
    strava = current_app.config["STRAVA"]
    from fuckzwift.web.routes_ui import _build_callback_url
...
def api_activities():
    config = current_app.config["APP_CONFIG"]
    from fuckzwift.storage.activities import list_activities
...
def _save_strava_to_toml(client_id: str, client_secret: str):
    ...
    from fuckzwift.config import PROJECT_ROOT
